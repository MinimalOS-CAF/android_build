...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^minimal_") ; then
       MINIMAL_BUILD=$(echo -n $1 | sed -e 's/^minimal_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $MINIMAL_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       MINIMAL_BUILD=
    fi
    export MINIMAL_BUILD
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the MINIMAL model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch minimal_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the MINIMAL github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var MINIMAL_VERSION)
    ...
    if (adb shell getprop ro.minimal.device | grep -q "$MINIMAL_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $MINIMAL_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $MINIMAL_FIXUP_COMMON_OUT ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.minimal.device | grep -q "$MINIMAL_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/minimal/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
